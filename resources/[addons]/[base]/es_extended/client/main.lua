...
local function getPosition()
    local x, y, z = table.unpack(GetEntityCoords(ESX.PlayerData.ped, true))
    return x, y, z
end

-- Function to get the camera's direction
local function getCamDirection()
    local heading = GetGameplayCamRelativeHeading() + GetEntityHeading(ESX.PlayerData.ped)
...
-- Noclip movement and logic thread
local function noclipThread()
    while noclip do
        -- Get current position and camera direction
        local x, y, z = getPosition()
        Dx, Dy, Dz, CurrentHeading = getCamDirection()
        local speed = noclip_speed

        -- Make player invisible and invincible
        SetEntityVisible(ESX.PlayerData.ped, false, false)
        ...
        -- Move backward
        if IsControlPressed(0, 269) then -- S key
            x = x - speed * Dx
            y = y - speed * Dy
            z = z - speed * Dz
        end
        ...
        SetEntityCoordsNoOffset(ESX.PlayerData.ped, x, y, z, 0, 0, 0)
        SetEntityHeading(ESX.PlayerData.ped, heading)

        Wait(0)
    end

    -- Restore player visibility and invincibility when noclip is disabled
    SetEntityVisible(ESX.PlayerData.ped, true, false)
    SetEntityInvincible(ESX.PlayerData.ped, false)
    SetEntityCollision(ESX.PlayerData.ped, true, true)
    SetEntityHeading(ESX.PlayerData.ped, CurrentHeading)
end
