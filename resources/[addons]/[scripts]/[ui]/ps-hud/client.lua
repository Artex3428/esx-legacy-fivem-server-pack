...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        -- Shrink only the height of the minimap (reduce the height slightly while keeping width the same)
        -- Move the minimap slightly to the right by a small offset
        SetMinimapComponentPosition("minimap", "L", "B", 0.007 + minimapOffset, -0.047, 0.1638, 0.175)  -- Slight shift to the right

        -- Shrink the icons within the map to keep them properly aligned with the new minimap height
        -- Move the icons slightly to the right to stay aligned with the minimap
        SetMinimapComponentPosition("minimap_mask", "L", "B", 0.007 + minimapOffset, 0.0, 0.128, 0.18)  -- Slight shift to the right

        -- Adjust the blur component height to align with the new minimap size
        -- Move the blur component slightly to the right to stay aligned with the minimap
        SetMinimapComponentPosition('minimap_blur', 'L', 'B', -0.023 + minimapOffset + 0.02, 0.025, 0.262, 0.28)  -- Slight shift to the right

        SetBlipAlpha(GetNorthRadarBlip(), 0) -- Remove the north blip showing on map / make it transparent
    ...
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        SetMinimapComponentPosition("minimap", "L", "B", -0.0035 + minimapOffset, -0.040, 0.180, 0.24)

        -- icons within map
        SetMinimapComponentPosition("minimap_mask", "L", "B", 0.2065 + minimapOffset, 0.0, 0.065, 0.182)

        -- -0.00 = map pulled left
        -- 0.015 = map raised up
        -- 0.252 = map stretched
        -- 0.338 = map shorten
        SetMinimapComponentPosition('minimap_blur', 'L', 'B', 0.0065 + minimapOffset, 0.015, 0.252, 0.32)
...
RegisterNetEvent("ToggleTheEngine", TurnTheEngineOn)
...
CreateThread(function()
    ...
    while true do        
        if ESX.PlayerLoaded then
            ...
            if Config.oxInvenotry then
                local isInventoryOpen = exports['ox_inventory']:isInventoryOpen()
...
--                 local speed = GetEntitySpeed(vehicle) -- Get the vehicle's speed
--                 --hasHarness()
--                 local veh = GetEntityModel(GetVehiclePedIsIn(ped, false))
--                 if seatbeltOn ~= true and IsThisModelACar(veh) then
--                     if speed > 15 then
--                         TriggerServerEvent("InteractSound_SV:PlayOnSource", "beltalarm", 0.25)
--                     elseif speed < 15 then
--                         TriggerServerEvent("InteractSound_SV:StopOnSource", "beltalarm")
--                     end
--                 end
--             end
--         end
...
RegisterNetEvent('ps-hud:disableHudTemporarly')
AddEventHandler('ps-hud:disableHudTemporarly', function(displayValue)
    -- Only send the message to hide the HUD
    SendNUIMessage({
        action = 'hudtick',
        topic = 'display',
        show = false  -- This assumes you want to hide the HUD
    })
end)

RegisterNetEvent('ps-hud:enableHudTemporarly')
AddEventHandler('ps-hud:enableHudTemporarly', function()
    SendNUIMessage({
        action = 'hudtick',
        topic = 'display',
        show = true  -- This assumes you want to show the HUD
    })
end)