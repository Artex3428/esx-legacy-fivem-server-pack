...
AddEventHandler('esx_mechanicjob:hasEnteredMarker', function(zone)
	if zone == 'NPCJobTargetTowable' then
		CurrentAction     = 'start_mission_now_for'
		CurrentActionMsg  = TranslateCap('startMission')
		CurrentActionData = {}
	...
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
				...
				elseif CurrentAction == 'start_mission_now_for' then
					local playerPed = PlayerPedId()
					if ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
						if NPCOnJob then
